...
public class ProductController {
    ...
    @Operation(summary = "Tìm kiếm sản phẩm theo chi nhánh", description = """
            ...
            **Ví dụ sử dụng:**
            ...
            """)
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "Tìm kiếm thành công", content = @Content(mediaType = "application/json", schema = @Schema(implementation = PagedResponse.class), examples = @ExampleObject(value = """
            {
              "content": [
                {
                  "productId": 1,
            ...
            """))), @ApiResponse(responseCode = "400", description = "Tham số không hợp lệ (thiếu branchId hoặc giá trị không đúng)"), @ApiResponse(responseCode = "404", description = "Không tìm thấy chi nhánh với ID được cung cấp")})
    @GetMapping("/search")
    public ResponseEntity<PagedResponse<ProductSearchDTO>> searchProducts(@Parameter(description = "ID chi nhánh (bắt buộc)", required = true, example = "1") @RequestParam(required = true) Integer branchId,
                                                                          ...
                                                                          @Parameter(description = "Trường sắp xếp", example = "name", schema = @Schema(allowableValues = {"name", "price", "createdDate", "productType"})) @RequestParam(required = false, defaultValue = "name") String sortBy,

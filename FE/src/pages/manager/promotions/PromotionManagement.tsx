...
const PromotionManagement: React.FC = () => {
  ...
  window.scrollTo(0, 0);
  const [promotionTypes] = useState<any[]>([
    { promotionTypeId: 1, name: 'Giảm giá' },
    { promotionTypeId: 2, name: 'Voucher' },
  ]);

  // Fake data for testing
  const fakePromotions: Promotion[] = [
    {
      promotionId: 1,
      name: 'Khuyến mãi khai trương',
      description: 'Giảm giá đặc biệt cho ngày khai trương',
      code: 'KHAITRUONG',
    ...
    {
      promotionId: 2,
      name: 'Voucher tháng 3',
      description: 'Voucher giảm giá cho tháng 3',
      code: 'MARCH2024',
    ...
    },
    {
      ...
      description: 'Giảm giá cho các đơn hàng cuối tuần',
      code: 'WEEKEND',
      discountType: 'Giảm giá',
      discountValue: 30000,
  ...
  // Comment out API call for promotion types
  /*
  useEffect(() => {
  ...
  */
  ...
  const handleDelete = async (promotionId: number) => {
    ...
    */
    ...
    // Use fake data instead
    setPromotions((prev) => prev.map((p) => (p.promotionId === promotionId ? { ...p, isActive: false } : p)));
    message.success('Đã ngừng hoạt động khuyến mãi thành công!');
  };
  ...
  const handleFormSubmit = async (values: PromotionFormValues) => {
    ...
    */
    ...
    const newPromotion: Promotion = {
      promotionId:
        modalMode === 'edit' && editingPromotion
          ? editingPromotion.promotionId
          : Math.max(...promotions.map((p) => p.promotionId)) + 1,
      name: values.name,
      description: values.description,
      code: values.code,
      discountType: promotionTypes.find((t) => t.promotionTypeId === values.promotionTypeId)?.name || '',
      discountValue: values.discountAmount,
      startDate: startDate.format('YYYY-MM-DDTHH:mm:ss'),
      endDate: endDate.format('YYYY-MM-DDTHH:mm:ss'),
      isActive: true,
      promotionTypeId: values.promotionTypeId,
    ...
    if (modalMode === 'edit' && editingPromotion) {
      setPromotions((prev) => prev.map((p) => (p.promotionId === editingPromotion.promotionId ? newPromotion : p)));
      message.success('Cập nhật khuyến mãi thành công!');
    } else {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_value: number, record: Promotion) => (
        <span style={{ color: '#A05A2C', fontWeight: 500 }}>{record.discountValue.toLocaleString()}đ</span>
      ),
    },
    {
      title: 'Thời gian',
    ...
    },
    {
      ...
      render: (_: any, record: Promotion) => {
        ...
        return (
          <Tag color={status.color} style={{ fontWeight: 500, padding: '3px 8px', borderRadius: '6px', color: '#fff' }}>
            {status.text}
          </Tag>
        );
      },
            ...
            <Card bordered={false} style={{ background: '#fff', borderRadius: 8, padding: 0 }}>
              ...
              >
                ...
                {selectedPromotion.code && (
                  <Descriptions.Item label='Mã khuyến mãi'>{selectedPromotion.code}</Descriptions.Item>
                )}
                <Descriptions.Item label='Giá trị giảm giá'>
                  {selectedPromotion.discountValue.toLocaleString()}đ
                </Descriptions.Item>
                <Descriptions.Item label='Ngày bắt đầu'>
                  {dayjs(selectedPromotion.startDate).format('DD/MM/YYYY HH:mm:ss')}
                </Descriptions.Item>
                <Descriptions.Item label='Ngày kết thúc'>
                  {dayjs(selectedPromotion.endDate).format('DD/MM/YYYY HH:mm:ss')}
                </Descriptions.Item>
            ...
            >
              ...
              <Form.Item
                ...
                rules={[
                  ...
                  { max: 10, message: 'Mã khuyến mãi không được vượt quá 10 ký tự!' },
                ]}
              >
                <Input placeholder='Ví dụ: SALE2025' style={{ borderRadius: 6}} />
              </Form.Item>
              <Form.Item
                name='promotionTypeId'
                label={<span style={{ color: '#A05A2C' }}>Loại khuyến mãi</span>}
              ...
              <Row gutter={[16, 16]}>
                <Col span={12}>
                  <Form.Item
                    name='discountAmount'
                    label={<span style={{ color: '#A05A2C' }}>Giá trị giảm</span>}
                ...
                <Col span={12}>
                  <Form.Item
                    ...
                    label={<span style={{ color: '#A05A2C' }}>Số lượt sử dụng tối đa</span>}
                    rules={[{ required: true, message: 'Vui lòng nhập số lượt sử dụng tối đa!' }]}
                  >

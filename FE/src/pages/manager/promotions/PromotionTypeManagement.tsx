    ...
    */
    ...
    const handleModalOk = async () => {
      ...
      try {
        ...
        if (editingType) {
          setPromotionTypes(prev => prev.map(pt => 
            pt.promotionTypeId === editingType.promotionTypeId 
              ? { ...pt, name: values.name }
              : pt
          ));
          message.success("Đã cập nhật loại khuyến mãi!");
        } else {
          const newType: PromotionType = {
            promotionTypeId: String(Math.max(...promotionTypes.map(pt => parseInt(pt.promotionTypeId))) + 1),
            name: values.name,
            usageCount: 0
          };
      ...
      <div style={{ maxWidth: 1300, margin: "40px auto 0 auto" }}>
        ...
        >
          <Form form={form} layout="vertical" initialValues={{ name: "" }}>
            ...
            >
              <Input placeholder="Ví dụ: Giảm giá theo %" style={{ borderRadius: 6 }} />
            </Form.Item>
          </Form>
        </Modal>
      </div>

...
const initialFeedbackData: Feedback[] = [
  ...
  },
  {
    ...
    content: "Đơn hàng của tôi đã được đặt từ 3 ngày trước nhưng vẫn chưa nhận được. Mong quý công ty kiểm tra và phản hồi sớm.",
    receivedDate: dayjs().subtract(1, "day").toISOString(),
    status: "in_progress",
    replyContent: "Chúng tôi xin lỗi vì sự chậm trễ này. Đơn hàng của anh đang được xử lý và sẽ được giao trong ngày mai.",
    repliedDate: dayjs().subtract(12, "hour").toISOString(),
    repliedBy: "CSKH Team",
  },
  {
    feedbackId: "FB006",
    customerName: "Hoàng Thị Hương",
    customerPhone: "0912345678",
    feedbackType: "suggestion",
    subject: "Đề xuất thêm phương thức thanh toán",
    content: "Tôi đề xuất quý công ty nên thêm phương thức thanh toán qua ví điện tử như Momo, ZaloPay để tiện lợi hơn cho khách hàng.",
    receivedDate: dayjs().subtract(4, "day").toISOString(),
    status: "resolved",
    replyContent: "Cảm ơn chị Hương về đề xuất rất hữu ích. Chúng tôi đang trong quá trình tích hợp thêm các phương thức thanh toán mới.",
    repliedDate: dayjs().subtract(3, "day").toISOString(),
    repliedBy: "Quản lý cửa hàng",
  },
  {
    feedbackId: "FB007",
    customerName: "Lê Văn Sơn",
    customerEmail: "son.lv@example.com",
    feedbackType: "compliment",
    subject: "Khen ngợi về chính sách đổi trả",
    content: "Tôi rất ấn tượng với chính sách đổi trả hàng của quý công ty. Quy trình đơn giản, nhanh chóng và nhân viên rất nhiệt tình hỗ trợ.",
    receivedDate: dayjs().subtract(2, "day").toISOString(),
    status: "closed",
    replyContent: "Cảm ơn anh Sơn đã dành thời gian chia sẻ phản hồi tích cực. Chúng tôi sẽ tiếp tục cải thiện dịch vụ để phục vụ khách hàng tốt hơn.",
    repliedDate: dayjs().subtract(1, "day").toISOString(),
    repliedBy: "CSKH Team",
  },
  {
    feedbackId: "FB008",
    ...
    content: "Tôi muốn biết thêm thông tin về chương trình khuyến mãi tháng này. Có áp dụng cho tất cả sản phẩm không?",
    receivedDate: dayjs().subtract(6, "hour").toISOString(),
    status: "new",
  }
];
...
const CustomerFeedbackManagement: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // TODO: Implement API call to fetch feedback data
    // Example API call:
    // try {
  ...
  const handleReplySubmit = async () => {
    ...
    try {
      ...
      // Example API call:
      // try {
      //   await axios.put(`/api/feedbacks/${selectedFeedback.feedbackId}`, updatedFeedback);
      ...
      // Using fake data update for now

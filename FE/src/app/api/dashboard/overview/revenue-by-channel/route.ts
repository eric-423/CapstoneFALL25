...
export async function GET(request: NextRequest) {
    try {
        ...
        const url = `${baseUrl}/dashboard/overview/revenue-by-channel${queryString ? `?${queryString}` : ''}`;

        console.log('🔄 Fetching Revenue by Channel:', url);
        ...
        if (!response.ok) {
            ...
            try {
                ...
                errorData = { error: errorText || 'Failed to fetch revenue by channel' };
            }

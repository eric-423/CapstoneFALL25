...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error(
        "❌ [Branch Orders API] Error response status:",
        response.status
      );
      ...
      console.error(
        "❌ [Branch Orders API] User role when error occurred:",
        decodedToken?.role || "Unknown"
      );
      console.error(
        "❌ [Branch Orders API] This endpoint may require MANAGER/ADMIN role, but user has:",
        decodedToken?.role || "Unknown"
      );

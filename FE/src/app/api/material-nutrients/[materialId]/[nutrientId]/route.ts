...
// DELETE /api/material-nutrients/{materialId}/{nutrientId}
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ materialId: string; nutrientId: string }> }) {
    try {
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ materialId: string; nutrientId: string }> }) {
    try {
        const cookieStore = await cookies();
        const token = cookieStore.get('token')?.value;

...
export function ProductForm({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [types, materialsData, cookingMethodsData, unitsData] =
          await Promise.all([
            getProductType(),
          ...
          getRecipesByProductId(product.productId)
            .then((recipes: ProductRecipes[]) => {
              const formattedRecipes = recipes
                ...
                .map((r) => {
                  const recipe = r as ProductRecipes & {
                    materialId?: number;
                    cookingMethodId?: number;
                  };
                  return {
                    materialId: recipe.materialId ?? recipe.material?.id ?? 0,
                    quantity: r.quantity,
  ...
  const onSubmit = async (data: ProductFormData) => {
    try {
      ...
      if (selectedFile) {
        try {
          ...
          if (
            ...
            toast.error(
              `Lỗi: ${errorMessage}. Bucket hiện tại: "${PRODUCT_BUCKET}"`,
              { autoClose: 8000 }
            );
          } else {
      ...
      const requestData: ProductCreateRequest = {
        ...
        typeId: data.typeId,
        recipesRequests:
          data.recipesRequests?.map((r, index) => ({
                ...
                <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-col items-center gap-4">
                      <div
                        className="relative w-full aspect-square border-2 border-dashed border-gray-300 rounded-xl flex items-center justify-center overflow-hidden bg-gray-50 hover:bg-gray-100 transition-colors group cursor-pointer"
                        onClick={() =>
                  ...
                  <div className="space-y-2">
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        ...
                        >
                          <SelectTrigger
                            className={`focus:border-[#78A243] focus:ring-[#78A243]/20 ${errors.typeId ? "border-red-500" : ""}`}
                          >

...
export function WarehouseFormDialog({
  ...
  useEffect(() => {
    ...
    setFormData({
      address:
        initialAddress ||
        (branchExists
          ? (branches.find((b) => b.id === initialBranchId)?.address ?? "")
          : ""),
  ...
  useEffect(() => {
    ...
    if (branchExists && formData.branchId !== String(initialBranchId)) {
      setFormData((prev) => ({
        ...prev,
        branchId: String(initialBranchId),
        address:
          prev.address ||
          (branches.find((b) => b.id === initialBranchId)?.address ?? ""),
      }));
  ...
  let availableBranches = branches.filter((branch) => {
    ...
    if (warehouse && String(warehouse.branchId) === String(branch.id))
      return true;
    // If initialBranchId is provided (quick-create), keep that branch even if it's in existingBranchIds
    if (initialBranchId && String(branch.id) === String(initialBranchId))
      return true;
  ...
  if (warehouse && warehouse.branchId) {
    ...
    const hasCurrentBranch = availableBranches.some(
      (b) => String(b.id) === currentBranchId
    );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const duplicateWarehouse = warehouses.find(
      (w) =>
    ...
    if (duplicateWarehouse) {
      toast.error(
        `Địa chỉ này đã được sử dụng bởi kho khác (ID: ${duplicateWarehouse.id})!`
      );
      return;
    }
    ...
    if (duplicateBranch && String(duplicateBranch.id) !== formData.branchId) {
      toast.error(
        `Địa chỉ này đã được sử dụng bởi chi nhánh "${duplicateBranch.name}"!`
      );
      return;
    }
              ...
              <Button
                ...
                onClick={() => {
                  const selectedBranch = branches.find(
                    (b) => b.id.toString() === formData.branchId
                  );
                  if (selectedBranch) {
                    setFormData({
                      ...formData,

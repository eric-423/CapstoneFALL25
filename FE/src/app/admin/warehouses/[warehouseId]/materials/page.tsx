...
export default function WarehouseMaterialsPage({
  ...
  const [selectedMaterial, setSelectedMaterial] =
    useState<WarehouseMaterial | null>(null);
  ...
  const filteredMaterials = useMemo(() => {
    const filtered = warehouseMaterials.filter((material) => {
      ...
      const matchesType =
        !typeFilter || material.materialTypeName === typeFilter;
    ...
    return [...filtered].sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case "materialTypeName":
          aValue = a.materialTypeName.toLowerCase();
          bValue = b.materialTypeName.toLowerCase();
          break;
        case "quantity":
          aValue = a.quantity;
          bValue = b.quantity;
        ...
        case "threshold":
          ...
          break;
        case "isLowStock":
          aValue = a.quantity < a.threshold ? 1 : 0;
          bValue = b.quantity < b.threshold ? 1 : 0;
          break;
        default:
          aValue = a.materialName.toLowerCase();
          bValue = b.materialName.toLowerCase();
  ...
  return (
    <AdminPageLayout>
      ...
      {/* Filters */}
      <div className="flex flex-wrap items-center justify-between gap-3 p-3 bg-white backdrop-blur-sm border-gray-300 border shadow-sm rounded-xl mb-4">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-white border-b-2 border-grey-300">
              <tr>
                <th
                  className="px-4 py-3 text-center text-sm font-bold text-[#2D1E1A] uppercase tracking-wider cursor-pointer hover:bg-gray-50 transition-colors"
                ...
                <th
                  className="px-4 py-3 text-center text-sm font-bold text-[#2D1E1A] uppercase tracking-wider cursor-pointer hover:bg-gray-50 transition-colors"
                  onClick={() => handleSort("isLowStock")}
                >
                  <div className="flex items-center justify-center gap-1.5">
                    Trạng thái
                    {getSortIcon("isLowStock")}
                  </div>
                </th>

...
import { X, Upload, Download, FileSpreadsheet, AlertCircle, CheckCircle, Trash2 } from 'lucide-react';
...
export function ImportMaterialDialog({
    ...
    const handleDownloadTemplate = async () => {
        try {
            ...
            selectableMaterials.forEach(material => {
                worksheet.addRow({ material: material.name, quantity: '', threshold: '' });
            });

            // If no materials available, add sample rows
            if (selectableMaterials.length === 0) {
                worksheet.addRow({ material: 'Ví dụ: Thịt bò', quantity: 100, threshold: 10 });
                worksheet.addRow({ material: 'Ví dụ: Rau xanh', quantity: 50, threshold: 5 });
            }
    ...
    const validateImportData = (rows: ImportRow[]): ImportRow[] => {
        ...
        return rows.map(row => {
            ...
            // Check threshold
            if (row.threshold === null || row.threshold === undefined || isNaN(row.threshold)) {
        ...
        try {
            ...
            worksheet.eachRow((row, rowNumber) => {
                ...
                const quantity = quantityValue !== null && quantityValue !== undefined && quantityValue !== ''
                    ...
                    : null;
                const threshold = thresholdValue !== null && thresholdValue !== undefined && thresholdValue !== ''
    ...
    const handleRemoveRow = (index: number) => {
        ...
        const revalidated = validateImportData(newData.map(row => ({
            ...row,
            errors: [],
            isValid: false
        })));
            ...
            <Card className="w-full max-w-4xl bg-white shadow-2xl rounded-2xl overflow-hidden max-h-[90vh] flex flex-col py-0">
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    ...
                    <div className="mb-6 flex-shrink-0">
                        <h3 className="text-lg font-semibold text-[#2D1E1A] mb-3 flex items-center gap-2">
                            <span className="w-6 h-6 bg-[#78A243] text-white rounded-full flex items-center justify-center text-sm">1</span>
                            Tải file mẫu
                        </h3>
                        <div className="flex items-center gap-4 p-4 bg-[#78A243]/10 rounded-lg border border-[#78A243]/30">
                            <div className="flex-1">
                                <p className="text-sm text-[#2D1E1A]">
                                    File mẫu đã có sẵn danh sách <strong>{selectableMaterials.length}</strong> nguyên liệu chưa có trong kho.
                                    Bạn chỉ cần điền số lượng và ngưỡng cảnh báo.
                                </p>
                            </div>
                            <Button
                    ...
                    <div className="mb-6 flex-shrink-0">
                        <h3 className="text-lg font-semibold text-[#2D1E1A] mb-3 flex items-center gap-2">
                            <span className="w-6 h-6 bg-[#78A243] text-white rounded-full flex items-center justify-center text-sm">2</span>
                            Upload file Excel
                        </h3>
                        <div
                        ...
                        >
                            ...
                            {fileName ? (
                                ...
                                <>
                                    <p className="text-[#2D1E1A] font-medium">Click để chọn file hoặc kéo thả vào đây</p>
                                    <p className="text-sm text-gray-500 mt-1">Hỗ trợ file .xlsx, .xls</p>
                                </>
                            )}
                        </div>
                    </div>
                    ...
                    {importData.length > 0 && (
                        <div className="flex-shrink-0">
                            <h3 className="text-lg font-semibold text-[#2D1E1A] mb-3 flex items-center gap-2">
                                <span className="w-6 h-6 bg-[#78A243] text-white rounded-full flex items-center justify-center text-sm">3</span>
                                Xem trước dữ liệu
                            ...
                            <div className="border rounded-lg overflow-hidden">
                                <table className="w-full">
                                    <thead className="bg-gray-50">
                                        <tr>
                                            <th className="px-4 py-3 text-left text-sm font-bold text-[#2D1E1A]">STT</th>
                                            <th className="px-4 py-3 text-left text-sm font-bold text-[#2D1E1A]">Nguyên liệu</th>
                                            <th className="px-4 py-3 text-left text-sm font-bold text-[#2D1E1A]">Số lượng</th>
                                            <th className="px-4 py-3 text-left text-sm font-bold text-[#2D1E1A]">Ngưỡng</th>
                                            <th className="px-4 py-3 text-left text-sm font-bold text-[#2D1E1A]">Trạng thái</th>
                                            <th className="px-4 py-3 text-center text-sm font-bold text-[#2D1E1A]">Xóa</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-200">
                                        {importData.map((row, index) => (
                                            <tr
                                                key={index}
                                                className={row.isValid ? 'bg-green-50/50' : 'bg-red-50/50'}
                                            >
                                                <td className="px-4 py-3 text-sm text-gray-600">{index + 1}</td>
                                                <td className="px-4 py-3 text-sm font-medium text-[#2D1E1A]">
                                                    {row.materialName || <span className="text-red-500 italic">Trống</span>}
                                                </td>
                                                <td className="px-4 py-3 text-sm text-[#2D1E1A]">
                                                    {row.quantity !== null ? row.quantity : <span className="text-red-500 italic">Trống</span>}
                                                </td>
                                                <td className="px-4 py-3 text-sm text-[#2D1E1A]">
                                                    {row.threshold !== null ? row.threshold : <span className="text-red-500 italic">Trống</span>}
                                                </td>
                                                    ...
                                                    {row.isValid ? (
                                                        ...
                                                        <div className="flex flex-col gap-1">
                                                            {row.errors.map((error, i) => (
                                                                <Badge key={i} className="bg-red-100 text-red-700 border-red-300 text-xs">
                ...
                <div className="p-6 bg-gray-50 border-t border-gray-200 flex gap-3 justify-between flex-shrink-0">
                    <div className="text-sm text-gray-600">
                        {importData.length > 0 && (
                            <>
                                Tổng: <strong>{importData.length}</strong> dòng |
                                Hợp lệ: <strong className="text-green-600">{validCount}</strong> |
                                Lỗi: <strong className="text-red-600">{invalidCount}</strong>
                            </>
                        )}
                    </div>

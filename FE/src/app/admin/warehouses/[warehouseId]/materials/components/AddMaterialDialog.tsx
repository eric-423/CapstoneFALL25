                    ...
                    <div className="p-6 overflow-y-auto flex-1">
                        ...
                        <div className="flex gap-3 mt-4">
                            <Button
                                ...
                                variant="outline"
                                className="flex-1 border-2 border-dashed border-[#78A243]/30 hover:border-[#78A243] hover:bg-[#78A243]/10"
                                disabled={loading}
                            >
                            ...
                            <Button
                                type="button"
                                onClick={() => {
                                    // Get currently selected material IDs from the form
                                    const currentSelectedIds = materials
                                        .map(m => m.materialId)
                                        .filter(id => id !== '');

                                    // Find materials that are available (not in existing warehouse materials AND not currently selected in form)
                                    const remainingMaterials = selectableMaterials.filter(
                                        m => !currentSelectedIds.includes(m.id.toString())
                                    );
                                    ...
                                    // Create new rows for remaining materials
                                    const newRows = remainingMaterials.map(m => ({
                                    ...
                                    const lastRow = materials[materials.length - 1];
                                    const isLastRowEmpty = !lastRow.materialId && !lastRow.quantity && !lastRow.threshold;

                                    if (isLastRowEmpty) {
                                        setMaterials([...materials.slice(0, -1), ...newRows]);
                                    } else {
                            ...
                            >
                                <Plus className="h-4 w-4 mr-2" />
                                Thêm tất cả ({selectableMaterials.length - materials.filter(m => m.materialId).length})
                            </Button>
                        </div>
                    </div>

          ...
          <div className="p-6 overflow-y-auto flex-1">
            <div className="space-y-4">
              {materials.map((material, index) => (
                <div
                  key={index}
                  className="border-2 border-[#78A243]/20 rounded-lg p-4"
                >
                      ...
                      {material.materialId &&
                        (() => {
                          const selectedMaterial = availableMaterials.find(
                            (m) => m.id.toString() === material.materialId
                          );
                          const unitName = selectedMaterial
                            ? units.find(
                                (u) => u.id === selectedMaterial.unitId
                              )?.name
                            : "";
                          return unitName ? (
            ...
            <div className="flex gap-3 mt-4">
              ...
              >
                <Plus className="h-4 w-4 mr-2" />
                Thêm tất cả (
                {selectableMaterials.length -
                  materials.filter((m) => m.materialId).length}
                )
              </Button>
            </div>
          </div>

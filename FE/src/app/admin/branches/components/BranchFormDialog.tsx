...
export function BranchFormDialog({
  ...
  const handleSubmit = async () => {
    ...
    if (duplicateBranch) {
      toast.error(
        `Địa chỉ này đã được sử dụng bởi chi nhánh khác (ID: ${duplicateBranch.id})!`
      );
      return;
    }

    // Check for duplicate address in warehouses
    const duplicateWarehouse = existingWarehouses.find(
      (w) => w.address.trim().toLowerCase() === normalizedAddress
    );

    if (duplicateWarehouse) {
      toast.error(
        `Địa chỉ này đã được sử dụng bởi kho (ID: ${duplicateWarehouse.id})!`
      );
      return;
    }

...
export default function ComboEffectivenessChart({
  data,
  isLoading,
}: ComboEffectivenessChartProps) {
  const hasData =
    data && data.length > 0 && data.some((d) => d.orders > 0 || d.revenue > 0);
  ...
  return (
    <div className="bg-white backdrop-blur-sm p-6 rounded-xl shadow-sm border border-grey-300 h-full">
      ...
      <p className="text-sm text-[#2D1E1A]/60 mb-6">
        So sánh số lượng bán và doanh thu của các Combo
      </p>
          ...
          <div className="w-full h-full flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-xs text-[#2D1E1A]/50 mt-1">
                Hiệu quả combo sẽ hiển thị tại đây
              </p>
            </div>
          </div>
        ) : (

      ...
      <div className="space-y-3 overflow-y-auto flex-1 min-h-0 pr-2 scrollbar-thin scrollbar-thumb-[#78A243]/20 scrollbar-track-transparent">
        {isLoading ? (
          Array.from({ length: 5 }).map((_, index) => (
            <div
              key={index}
              className="flex items-center gap-4 p-3 rounded-lg animate-pulse bg-[#EBD187]/10"
            >
          ...
          <div className="flex-1 flex items-center justify-center py-12">
            <div className="text-center">
              ...
              <p className="text-xs text-[#2D1E1A]/50 mt-1">
                Dữ liệu sản phẩm bán chạy sẽ hiển thị tại đây
              </p>
            </div>
          </div>
        ) : (
          data.map((product, index) => (
            <div
              ...
              className="flex items-center gap-4 p-3 rounded-lg hover:bg-[#78A243]/5 transition-colors border border-transparent hover:border-[#78A243]/10"
            >
              ...
              <div className="flex-1 min-w-0">
                <h4 className="text-sm font-medium text-[#2D1E1A] truncate">
                  {product.productName}
                </h4>

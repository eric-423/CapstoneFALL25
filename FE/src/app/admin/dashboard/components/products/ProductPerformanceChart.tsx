...
export default function ProductPerformanceChart({ data, isLoading }: ProductPerformanceChartProps) {
    const hasData = data && data.length > 0 && data.some(d => d.totalRevenue > 0 || d.quantitySold > 0);

    // Calculate Y-axis domain based on data
    const yDomain = useMemo(() => {
        if (!data || data.length === 0) {
            return [0, 10000000]; // Default domain when no data: 0 - 10M
        }
        const maxRevenue = Math.max(...data.map(d => d.totalRevenue || 0));
        if (maxRevenue === 0) {
            return [0, 10000000]; // Default when all values are 0
        }
        // Add 10% padding on top
        return [0, Math.ceil(maxRevenue * 1.1)];
    }, [data]);

    // Calculate X-axis domain based on data
    const xDomain = useMemo(() => {
        ...
        if (maxQuantity === 0) {
            return [0, 100]; // Default when all values are 0
        }
        // Add 10% padding on top
    ...
    return (
        <div className="bg-gradient-to-br from-white/80 to-[#EBD187]/10 backdrop-blur-sm p-6 rounded-xl shadow-sm border border-[#78A243]/20 h-full">
            <div className="flex items-center gap-2 mb-1">
                ...
                <h3 className="text-lg font-semibold text-[#2D1E1A]">Hiệu Suất Sản Phẩm</h3>
                {isLoading && <Loader2 className="h-4 w-4 animate-spin text-[#78A243]" />}
            </div>
            <p className="text-sm text-[#2D1E1A]/60 mb-6">Tương quan giữa Số lượng bán và Doanh thu</p>
                ...
                {isLoading ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                        <div className="text-center">
                            ...
                            <p className="text-sm font-medium text-[#2D1E1A]/70">Chưa có dữ liệu hiệu suất</p>
                            <p className="text-xs text-[#2D1E1A]/50 mt-1">Biểu đồ tương quan sẽ hiển thị tại đây</p>
                        </div>
                    </div>
                ) : (
                    <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                            <CartesianGrid strokeDasharray="3 3" stroke="#78A243" strokeOpacity={0.2} />
                            <XAxis
                                type="number"
                                ...
                                unit=" món"
                                domain={xDomain}
                                label={{ value: 'Số lượng bán', position: 'bottom', offset: 0, fill: '#2D1E1A', fontSize: 12, fillOpacity: 0.6 }}
                            ...
                            <YAxis
                                ...
                                name="Doanh thu"
                                domain={yDomain}
                                label={{ value: 'Doanh thu', angle: -90, position: 'left', offset: 0, fill: '#2D1E1A', fontSize: 12, fillOpacity: 0.6 }}
                                tick={{ fill: '#2D1E1A', fontSize: 12 }}
                                    ...
                                    if (active && payload && payload.length) {
                                        ...
                                        return (
                                            <div className="bg-white/95 backdrop-blur-sm border border-[#78A243]/20 p-3 rounded-lg shadow-lg">
                                                <p className="font-semibold text-[#2D1E1A] mb-1">{data.productName}</p>
                                                <p className="text-sm text-[#2D1E1A]/60">Bán: {data.quantitySold}</p>

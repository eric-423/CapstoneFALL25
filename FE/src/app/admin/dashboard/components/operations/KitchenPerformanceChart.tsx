...
export default function KitchenPerformanceChart({ data, isLoading }: KitchenPerformanceChartProps) {
    const hasData = data && data.length > 0 && data.some(d => d.avgPrepMinutes > 0);
    ...
    return (
        <div className="bg-gradient-to-br from-white/80 to-[#EBD187]/10 backdrop-blur-sm p-6 rounded-xl shadow-sm border border-[#78A243]/20">
            ...
            <p className="text-sm text-[#2D1E1A]/60 mb-6">Thời gian chế biến trung bình theo khung giờ</p>
                ...
                {isLoading ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                        <div className="text-center">
                            ...
                            <p className="text-sm font-medium text-[#2D1E1A]/70">Chưa có dữ liệu bếp</p>
                            <p className="text-xs text-[#2D1E1A]/50 mt-1">Hiệu suất bếp sẽ hiển thị tại đây</p>
                        </div>
                    </div>
                ) : (
                    <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#78A243" strokeOpacity={0.2} />
                            <XAxis
                                dataKey="timeSlot"
                                axisLine={false}
                                tickLine={false}
                            ...
                            <YAxis
                                ...
                                tickLine={false}
                                tick={{ fill: '#2D1E1A', fontSize: 12 }}
                                label={{ value: 'Phút', angle: -90, position: 'insideLeft', fill: '#2D1E1A', fillOpacity: 0.6 }}
                            />
                            <Tooltip
                            ...
                            />
                            <ReferenceLine y={15} stroke="#DA7339" strokeDasharray="3 3" label={{ value: 'Mục tiêu (15p)', fill: '#DA7339', fontSize: 10 }} />
                            <Bar

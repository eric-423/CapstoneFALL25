...
export default function PromotionPerformanceChart({ data, isLoading }: PromotionPerformanceChartProps) {
    const hasData = data && data.length > 0 && data.some(d => d.revenueGenerated > 0);
    ...
    return (
        <div className="bg-gradient-to-br from-white/80 to-[#EBD187]/10 backdrop-blur-sm p-6 rounded-xl shadow-sm border border-[#78A243]/20 h-full">
            <div className="flex items-center gap-2 mb-1">
                ...
                <h3 className="text-lg font-semibold text-[#2D1E1A]">Hiệu Quả Khuyến Mãi</h3>
                {isLoading && <Loader2 className="h-4 w-4 animate-spin text-[#78A243]" />}
            </div>
            <p className="text-sm text-[#2D1E1A]/60 mb-6">Doanh thu tạo ra từ các mã giảm giá</p>
                ...
                {isLoading ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                        <div className="text-center">
                            ...
                            <p className="text-sm font-medium text-[#2D1E1A]/70">Chưa có dữ liệu khuyến mãi</p>
                            <p className="text-xs text-[#2D1E1A]/50 mt-1">Hiệu quả khuyến mãi sẽ hiển thị tại đây</p>
                        </div>
                    </div>
                ) : (

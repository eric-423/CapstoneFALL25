...
export default function VoucherRevenueChart({
  data,
  isLoading,
}: VoucherRevenueChartProps) {
  const totalRevenue = data.reduce((acc, curr) => acc + curr.revenue, 0);
  const hasData = data && data.length > 0 && totalRevenue > 0;

  return (
    <div className="bg-white backdrop-blur-sm p-6 rounded-xl shadow-sm border border-grey-300 h-full flex flex-col">
      ...
      <p className="text-sm text-[#2D1E1A]/60 mb-4">
        Tỷ trọng doanh thu theo loại Voucher
      </p>
        ...
        <div className="flex-1 min-h-0 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-xs text-[#2D1E1A]/50 mt-1">
              Doanh thu voucher sẽ hiển thị tại đây
            </p>
          </div>
        </div>
      ) : (
        <>
          <div className="flex-1 min-h-0 relative">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <span className="text-sm text-[#2D1E1A]/60 font-medium">
                Tổng
              </span>
          ...
          <div className="mt-4 space-y-3">
            {data.map((item, index) => {
              ...
              return (
                <div
                  key={index}
                  className="flex items-center justify-between p-2 rounded-lg hover:bg-[#78A243]/5 transition-colors"
                >
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-sm font-medium text-[#2D1E1A]">
                      {item.type}
                    </span>
                  </div>

  ...
  useEffect(() => {
    const fetchTraining = async () => {
      ...
      try {
        ...
        const response = await getTrainningById(trainingId);
        const training = (response.data || response) as TrainingCourse;

        if (!training) {
          toast.error("Không tìm thấy khóa đào tạo");
          router.push("/admin/training");
          return;
        }
        ...
        const serverDesc =
          ...
          (error instanceof Error
            ? error.message
            : "Không thể tải thông tin khóa đào tạo");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Cập nhật khóa đào tạo thành công!", {
        toastId: `update-training-${trainingId}`,
      });
      router.push("/admin/training");
    } catch (error) {
      const serverDesc =
      ...
      toast.error(serverDesc, {
        toastId: `training-error-edit-${trainingId}`,
      });
    } finally {
      setIsLoading(false);
    }
        ...
        <AdminPageLayout>
          <div className="flex items-center justify-center py-16">
            <div className="flex items-center gap-3 text-gray-500">
              ...
              <span>Đang tải thông tin khóa đào tạo...</span>
            </div>
          </div>
        </AdminPageLayout>
      </AdminGuard>
    ...
    <AdminGuard>
      <AdminPageLayout>
        <AdminPageHeader
          ...
          description="Chỉnh sửa thông tin khóa đào tạo"
          icon={GraduationCap}
          actions={
            <Button
              variant="outline"

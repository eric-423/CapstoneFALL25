...
const isStaffRole = (role: StaffRole | undefined): role is StaffRole =>
  Boolean(role);
...
export default function TrainingPage() {
  ...
  const [lessonDocumentsError, setLessonDocumentsError] = useState<
    string | null
  >(null);
  ...
  const handleDocumentFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (lessonDetailState.lesson) {
        try {
          setLessonDocumentsLoading(true);
          const documentsResponse = await getLessonDocuments(
            lessonDetailState.lesson.id
          );
          setLessonDocuments(documentsResponse?.data ?? []);
        } catch (docError) {
  ...
  const validateLessonForm = () => {
    ...
    if (!lessonForm.point.trim()) {
      ...
      Number.isNaN(Number(lessonForm.point)) ||
      Number(lessonForm.point) <= 0
    ) {
    ...
    if (!lessonForm.orderIndex.trim()) {
      ...
      Number.isNaN(Number(lessonForm.orderIndex)) ||
      Number(lessonForm.orderIndex) <= 0
    ) {
    ...
    <AdminGuard>
      <AdminPageLayout>
        ...
        <Dialog
          ...
          onOpenChange={(open: boolean) => (open ? null : closeDetailDialog())}
        >
                  ...
                  <div className="border-t border-gray-200 pt-4 space-y-4 ">
                    ...
                    {lessonPanelView === "list" ? (
                      lessonsLoading ? (
                        ...
                        <p className="text-sm text-gray-500">
                          Chưa có bài học nào cho khóa này.
                        </p>
                      ) : (
                          ...
                          <div className="space-y-4">
                            <div className="flex flex-col gap-1">
                              <p className="text-xl font-bold text-gray-900">
                                {lessonDetailState.lesson.title}
                              </p>
                              <p className="text-sm text-gray-500">
                                Thứ tự #{lessonDetailState.lesson.orderIndex} ·{" "}
                                {lessonDetailState.lesson.isActive
                                  ? "Đang hoạt động"
                                  : "Đã ẩn"}
                              </p>
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                              {[
                                {
                                  label: "Điểm bài học",
                                  value: lessonDetailState.lesson.point,
                                },
                                {
                                  label: "Thuộc khóa",
                                  value: lessonDetailState.lesson.trainingId,
                                },
                                {
                                  label: "ID",
                                  value: lessonDetailState.lesson.id,
                                },
                              ].map((stat) => (
                                <Card
                                  key={stat.label}
                            ...
                            <div className="space-y-2 border-t border-gray-200 pt-4">
                              <div className="flex items-center justify-between">
                                ...
                                <Button
                                  ...
                                  disabled={
                                    lessonDocumentsLoading || documentFormOpen
                                  }
                                    ...
                                    <div className="space-y-3">
                                      {lessonDocuments.map((doc) => {
                                        ...
                                        return (
                                          <Card
                                            key={doc.id}
                                            className={`p-4 border transition-colors bg-white ${isEditingThisDoc ? "border-2 border-primary" : "border-gray-200 hover:border-primary"}`}
                                          >
                                            {isEditingThisDoc ? (
                                              <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-3 pt-2">
                    <button
                      aria-label={
                        lessonForm.isActive ? "Ẩn bài học" : "Kích hoạt bài học"
                      }
                      type="button"

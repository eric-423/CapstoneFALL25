...
export const getTrainnings = async (data?: GetTrainningsData): Promise<TrainingResponse> => {
...
export const getMyTrainning = async (status?: 'ACTIVE' | 'INACTIVE' | 'ALL') => {
    const params = new URLSearchParams();
...
export const createTraining = async (payload: CreateTrainingPayload): Promise<TrainingResponse> => {
...
export const getLessonDetail = async (lessonId: number): Promise<LessonDetailResponse> => {
...
export const deleteLessonDocument = async (documentId: number): Promise<{ status: number; desc: string; data: null }> => {
    const response = await fetch(`/api/documents/admin/${documentId}`, {
        method: 'DELETE',
        credentials: 'include',
        headers: {

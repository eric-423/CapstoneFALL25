...
export function ScheduleManagement({
  ...
  const isCurrentWeek = () => {
    ...
    currentMonday.setDate(
      currentMonday.getDate() - currentDay + (currentDay === 0 ? -6 : 1)
    );
    ...
    try {
      ...
      if (editingSchedule) {
        ...
        const tempSchedule: Schedule = {
          ...
          userName:
            users.find((u) => u.id === (payload as CreateScheduleData).userId)
              ?.name || "",
        ...
        if (createResponse.status !== 0 && createResponse.status !== 200) {
          ...
          if (
            errorDesc.includes(
              "Schedule for this user, shift and date already exists"
            ) ||
    ...
    return safeSchedules.filter((schedule) => {
      ...
      if (filterShift !== "all" && schedule.startTime) {
        ...
        const endHour = schedule.endTime
          ? parseInt(schedule.endTime.split(":")[0] || "0")
          : startHour;
      ...
      <div className="flex flex-col lg:flex-row gap-4 mb-4">
        ...
        <div className="flex-1 flex items-center justify-end gap-3">
          ...
          <Select value={filterShift} onValueChange={setFilterShift}>
            <SelectTrigger className="w-[200px] h-10 border-gray-200 focus:border-[#78A243] focus:ring-[#78A243]/20">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-3 sm:p-4 mb-6">
        {hasActiveFilters && (
          <div className="mb-3 px-1 flex items-center gap-2 text-sm text-gray-500">
            <span>
              Đang hiển thị {filteredSchedules.length} / {safeSchedules.length}{" "}
              lịch trình
            </span>
        ...
        <DialogContent className="sm:max-w-[420px]">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <br /> Lịch trình sẽ bị xóa vĩnh viễn.
            </DialogDescription>
          </DialogHeader>

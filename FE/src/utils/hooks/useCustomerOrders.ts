import { cancelOrder, GET_CUSTOMER_ORDER_QUERY_KEY, getCustomerOrders, OrderResponse, ComboDTO, OrderProductResponse } from '@/apis/order.api';
    ...
    const items = rawItems
        .map(
            (item: RawOrderItem) => {
                ...
                // Nếu là combo, sử dụng thông tin từ comboDTO
                if (isCombo && item.comboDTO) {
                    return {
                        productId: item.productId ?? item.product_id ?? item.comboDTO.id ?? 0,
                        productName: item.comboDTO.name ?? 'Combo',
                        quantity: item.quantity ?? item.qty ?? 0,
                        price: item.comboDTO.price ?? item.price ?? item.amount ?? 0,
                        note: item.note ?? '',
                        feedback: item.feedback ?? undefined,
                        comboDTO: item.comboDTO,
                        isCombo: true,
        ...
        .filter((item) => {
            // Giữ lại tất cả các item có productName hợp lệ hoặc là combo
            return item.isCombo || (item.productName && item.productName !== 'Sản phẩm');
        });
